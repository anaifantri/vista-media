...
class SalesDataController extends Controller
{
    ...
    { 
        ...
        $media_categories = MediaCategory::with('sales_data')->get();
        $salesData = SalesData::where('quotation_id', $id)->get();
        return view('sales-data.preview', [
            'quotation'=>$quotation,
    ...
    public function home(String $category, Request $request): View
    {
        if($category == "All"){
            ...
            $salesData = SalesData::filter(request('search'))->sortable()->category()->paginate(10)->withQueryString();
        }else{
            $dataCategory = MediaCategory::where('name', $category)->get()->last();
            $media_category_id = $dataCategory->id;
            $salesData = SalesData::where('media_category_id', $dataCategory->id)->filter(request('search'))->sortable()->paginate(10)->withQueryString();
        }

        $media_categories = MediaCategory::with('sales_data')->get();
        $companies = Company::with('sales_data')->get();
        $quotations = Quotation::with('sales_data')->get();
        return view ('sales-data.index', [
            'sales_data'=>$salesData,
            'categories'=>MediaCategory::all(),
    ...
    public function selectQuotation(String $category): View
    {
        if(auth()->user()->level === 'Administrator' || auth()->user()->level === 'Marketing'){
            ...
            $sales_data = SalesData::with('quotation')->get();
            return view ('sales-data.select-quotation', [
                'categories'=>MediaCategory::all(),
                ...
                compact('media_categories', 'companies', 'quotation_statuses', 'quotation_revisions', 'quot_revision_statuses', 'sales_data')
            ]);
        } else {
            abort(403);
        }
    ...
    public function createSales(String $category, String $quotationId){
        if(auth()->user()->level === 'Administrator' || auth()->user()->level === 'Marketing' ){
            ...
            return response()-> view ('sales-data.create', [
                'quotation'=>$quotation,
                'number'=>$number,
                'notes'=>$notes,
                'created_at'=>$created_at,
    ...
    public function store(Request $request): RedirectResponse
    {
        if(auth()->user()->level === 'Administrator' || auth()->user()->level === 'Marketing' ){
            ...
            return redirect('/sales-data/preview/'.$request->category.'/'.$validateData['quotation_id'])->with('success', 'Data penjualan berhasil ditambahkan');
        } else {
            abort(403);
        }
    }
     ...
     */
    public function show(SalesData $salesData): Response
    {
        dd($salesData);
    }
     ...
     */
    public function edit(SalesData $salesData): Response
    {
        //
    }
     ...
     */
    public function update(Request $request, SalesData $salesData): RedirectResponse
    {
        if($request->start_at && $request->end_at){
            if($request->start_at > $request->end_at){
                return redirect('/sales-data/home/'.$request->category)->with('failed','Awal > Akhir')->with('id', $request->sales_id);
            }else{
                $validateData['start_at'] = $request->start_at;
                $validateData['end_at'] = $request->end_at;

                SalesData::where('id', $request->sales_id)
                    ->update($validateData);
            }
            
            return redirect('/sales-data/home/'.$request->category)->with('success','Tanggal kontrak berhasil diupdate')->with('id', $request->sales_id);
        }else{
            return redirect('/sales-data/home/'.$request->category)->with('failed','Tanggal belum diinput')->with('id', $request->sales_id);
        }
    }
     ...
     */
    public function destroy(SalesData $salesData): RedirectResponse
    {
        //
    }
}
