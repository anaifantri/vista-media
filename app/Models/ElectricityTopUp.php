...
class ElectricityTopUp extends Model
{
    ...
    public function scopeYear($query){
        if (request('year')) {
            return $query->whereYear('topup_date', request('year'));
        }else{
            return $query->whereYear('topup_date', Carbon::now()->year);
        }
    }

    public function scopeMonth($query){
        if(request('month')){
            return $query->whereYear('topup_date', request('year'))->whereMonth('topup_date', request('month'));
        }else{
            return $query->whereYear('topup_date', Carbon::now()->year)->whereMonth('topup_date', Carbon::now()->month);
        }
    }
