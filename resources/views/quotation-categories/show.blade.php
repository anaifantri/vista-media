...
@section('container')
    <?php
    $createdDate = strtotime($quotation_category->created_at);
    $updatedDate = strtotime($quotation_category->updated_at);
    ...
    <div class="flex justify-center mt-10">
        <div class="p-4 w-[350px] h-[500px] border rounded-lg">
            <div class="flex justify-center items-center mb-3 w-full border-b">
                <h4 class="text-xl font-semibold tracking-wider text-teal-900">Detail Katagori
            ...
            <div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="border-b w-[280px]">
                        ...
                        <label class="flex text-semibold">{{ $quotation_category->code }}</label>
                    </div>
                </div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="border-b w-[280px]">
                        <label class="flex text-sm text-teal-700">Nama Katagori</label>
                        <label class="flex text-semibold">{{ $quotation_category->name }}</label>
                    </div>
                </div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="w-[280px]">
                        <label class="text-sm text-teal-700">Deskripsi</label>
                        <textarea class="flex text-semibold w-[280px] border rounded-lg p-1 outline-none" rows="4" readonly>{{ $quotation_category->description }}</textarea>
                    </div>
                </div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="border-b w-[280px]">
                ...
                @if ($quotation_category->created_at != $quotation_category->updated_at)
                    <div class="flex justify-center mt-2 w-full">
                        <div class="border-b w-[280px]">
                            <label class="flex text-sm text-teal-700">Diupdate Oleh</label>
                            <label class="flex text-semibold">{{ $quotation_category->user->name }}</label>
                        </div>
                    </div>
                @else
                    <div class="flex justify-center mt-2 w-full">
                ...
                <div class="flex justify-center mt-2">
                    <a href="/marketing/quotation-categories" class="flex items-center justify-center btn-primary mx-1">
                    ...
                    <a href="/marketing/quotation-categories/{{ $quotation_category->id }}/edit"
                    ...
                    <form action="/marketing/quotation-categories/{{ $quotation_category->id }}" method="post"
                        class="d-inline">
                        @method('delete')
                        @csrf
                        <button class="flex items-center justify-center btn-danger mx-1"
                            onclick="return confirm('Apakah anda yakin ingin menghapus data katagori pemasaran dengan nama {{ $quotation_category->name }} ?')">
                            <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path
