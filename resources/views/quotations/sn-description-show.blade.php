...
<div class="w-[256px] h-[170px] bg-slate-50 mt-1 rounded-b-lg border">
    ...
    <div class="flex mt-1">
        ...
        <span class="w-[120px] text-xs font-sans font-bold tracking-wide text-teal-900">:
            {{ $product->size }} - {{ $product->side }}</span>
    </div>
    <div class="flex mt-1">
    ...
    @if ($description->type == 'Videotron')
        <div class="flex mt-1">
            <span class="w-[90px] text-xs font-sans font-bold tracking-wide text-teal-900 ml-2">Screen
                Size</span>
            @if ($description->screen_w < $description->screen_h)
                <span class="w-[140px] text-xs font-sans font-bold tracking-wide text-teal-900">:
                    {{ $description->screen_w }} pixel x
                    {{ $description->screen_h }} pixel</span>
            @else
                <span class="w-[140px] text-xs font-sans font-bold tracking-wide text-teal-900">:
                    {{ $description->screen_h }} pixel x
                    {{ $description->screen_w }} pixel</span>
            @endif
        </div>
    @endif
    <div class="flex mt-1">
    ...
    @if ($description->type == 'Videotron')
        <div class="flex mt-1">
            <span class="w-[90px] text-xs font-sans font-bold tracking-wide text-teal-900 ml-2">Type
                LED</span>
            <span class="w-[140px] text-xs font-sans font-bold tracking-wide text-teal-900">:
                {{ $dataLed->pixel_pitch }} - {{ $dataLed->type }} -
                {{ $dataLed->pixel_config }}
            </span>
        </div>
        <div class="flex mt-1">
            <span class="w-[90px] text-xs font-sans font-bold tracking-wide text-teal-900 ml-2">Pixel
                Density</span>
            <span class="w-[140px] text-xs font-sans font-bold tracking-wide text-teal-900">:
        ...
        <div class="flex mt-1">
            <span class="w-[90px] text-xs font-sans font-bold tracking-wide text-teal-900 ml-2">Refresh
                Rate</span>
            <span class="w-[140px] text-xs font-sans font-bold tracking-wide text-teal-900">:
