...
@section('container')
    ...
    if (request('index_po')) {
        $indexPo = request('index_po');
    } else {
        $indexPo = 0;
    }
    if (count($quotation_orders) > 0) {
        $quotation_order = $quotation_orders[$indexPo];
        $showImages = json_decode($quotation_order->images);
    } else {
        $showImages = [];
    }
    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        ...
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            ...
            <div class="flex w-[950px] items-center border-b p-1">
                <h1 class="flex text-xl text-stone-100 font-bold tracking-wider w-[550px]">DOKUMEN PO</h1>
            ...
            <div class="grid grid-cols-2 gap-2 mt-2">
                ...
                <div class="border p-1 rounded-lg bg-stone-400">
                    <div class="flex w-full items-center border-b border-stone-900">
                        <label class="w-32 text-sm font-semibold m-1">Dokumen PO</label>
                    ...
                    <table class="table-auto w-full mt-2">
                        <thead>
                            <tr class="text-sm bg-stone-300">
                                <th class="border border-stone-900 w-36">No. PO</th>
                                <th class="border border-stone-900 w-[72px]">Tgl. PO</th>
                                <th class="border border-stone-900 w-8">Dok.</th>
                                <th class="border border-stone-900 w-16">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($quotation_orders as $quotationOrder)
                                @php
                                    $orderImages = json_decode($quotationOrder->images);
                                @endphp
                                <tr class="text-sm bg-stone-100">
                                    <td class="border border-stone-900 text-center">
                                        {{ $quotationOrder->number }}
                                    </td>
                                    <td class="border border-stone-900 text-center">
                                        {{ date('d', strtotime($quotationOrder->date)) }}-{{ $month[(int) date('m', strtotime($quotationOrder->date))] }}-{{ date('Y', strtotime($quotationOrder->date)) }}
                                    </td>
                                    <td class="border border-stone-900 text-center">
                                        {{ count($orderImages) }}
                                    </td>
                                    <td class="border border-stone-900 text-center">
                                        <div class="flex w-full justify-center items-center">
                                            <form class="m-1"
                                                action="/marketing/quotation-orders/show-orders/{{ $category }}/{{ $sale->id }}">
                                                <input type="text" name="index_po" value="{{ $loop->iteration - 1 }}"
                                                    hidden>
                                                <button type="submit"
                                            ...
                                            @canany(['isAdmin', 'isMarketing'])
                                                @can('isSale')
                                                    @can('isMarketingCreate')
                                                        <a href="/marketing/quotation-orders/{{ $quotationOrder->id }}/edit"
                                            ...
                                            @canany(['isAdmin', 'isMarketing'])
                                                @can('isSale')
                                                    @can('isMarketingDelete')
                                                        <form action="/marketing/quotation-orders/{{ $quotationOrder->id }}"
                                                            method="post" class="d-inline m-1">
                                                            @method('delete')
                                                            @csrf
                                                            <button
                                                                class="index-link text-white w-8 h-5 rounded bg-red-600 hover:bg-red-700 drop-shadow-md"
                                                                onclick="return confirm('Apakah anda yakin ingin menghapus dokumen po dengan nomor {{ $quotationOrder->number }} ?')">
                                                                <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd"
                                                                    stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24"
                                                                    xmlns="http://www.w3.org/2000/svg">
                                                                    <path
                                ...
                                <div class="divImage">
                                    <div class="grid grid-cols-2 gap-2">
                                        <div class="bg-stone-900 p-4 border rounded-lg">
                                            <div class="flex">
                                                <label class="text-sm text-yellow-400 w-28 mx-1">Nomor PO</label>
                                                <label class="text-sm text-yellow-400">:</label>
                                                <label
                                                    class="text-sm text-yellow-400 ml-2">{{ $quotation_order->number }}</label>
                                            </div>
                                            <div class="flex">
                                                <label class="text-sm text-yellow-400 w-28 mx-1">Tanggal PO</label>
                                                <label class="text-sm text-yellow-400">: </label>
                                                <label class="text-sm text-yellow-400 ml-2 w-40">
                                                    {{ date('d', strtotime($quotation_order->date)) }}
                                                    {{ $bulan[(int) date('m', strtotime($quotation_order->date))] }}
                                                    {{ date('Y', strtotime($quotation_order->date)) }}
                                                </label>
                                            </div>
                                        </div>
                                        <div class="bg-stone-900 p-4 border rounded-lg">
