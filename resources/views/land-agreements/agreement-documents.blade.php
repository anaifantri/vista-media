<div class="w-[950px] p-4">
    <div class="border-t border-b border-stone-900 p-1">
        <label class="flex justify-center text-sm font-semibold text-stone-900">DOKUMEN PERJANJIAN</label>
        <label class="flex justify-center text-sm font-semibold text-stone-900">Jumlah Dokumen :
            {{ count($agreements) }} dokumen</label>
    </div>
    <figure class="flex w-[950px] justify-center overflow-x-auto bg-stone-800 rounded-lg mt-2" id="figureAgreement">
        @foreach ($agreements as $document)
            @if (count($agreements) > 2)
                @if ($loop->iteration - 1 == intdiv(count($agreements), 2))
                    <img id="{{ $loop->iteration - 1 }}" class="photo-active"
                        ...
                        onclick="figureAction(this,'agreement')">
                @else
                    <img id="{{ $loop->iteration - 1 }}" class="photo" src="{{ asset('storage/' . $document->image) }}"
                        alt="" onclick="figureAction(this,'agreement')">
                @endif
            @else
                @if ($loop->iteration == 1)
                    <img id="{{ $loop->iteration - 1 }}" class="photo-active"
    ...
    <div class="relative m-auto w-[950px] h-max mt-2">
        <div class="absolute inset-y-0 left-0 w-7 h-12 m-auto">
            <button
                ...
                type="button" onclick="buttonPrev('agreement')">
                <svg class="fill-white w-5" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd"
                    viewBox="0 0 24 24">
        ...
        @foreach ($agreements as $document)
            @if (count($agreements) > 2)
                ...
                @else
                    <div class="divImageAgreement" hidden>
                        <div class="absolute top-2 left-0 w-full h-14 bg-black bg-opacity-80 p-2">
                            <div class="flex items-center">
                                <div class="w-64">
                                    <div class="flex">
