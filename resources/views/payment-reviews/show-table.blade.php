<table class="table-auto w-full">
    <thead>
        <tr>
            ...
            <th class="text-sm text-black border border-black w-28">Harga (Rp.)</th>
        </tr>
        <tr>
            <th class="text-sm text-black border border-black w-10" rowspan="2">Jenis</th>
            @if ($category == 'Signage')
                <th class="text-sm text-black border border-black w-16" rowspan="2">Bentuk</th>
            @else
                <th class="text-sm text-black border border-black w-12" rowspan="2">BL/FL</th>
            @endif
            @if ($category == 'Signage')
                <th class="text-sm text-black border border-black w-6" rowspan="2">Qty</th>
            @endif
            <th class="text-sm text-black border border-black w-10" rowspan="2">Side</th>
    ...
    <tbody>
        <tr>
            ...
            @if ($category == 'Signage')
                <td class="text-sm text-black border border-black text-center">{{ $description->type }}</td>
            @else
            ...
            <td id="previewPrice" class="text-sm  text-black border border-black text-right px-2">
        ...
        @if ($category != 'Videotron' || ($category == 'Signage' && $description->type != 'Videotron'))
            ...
            @if (isset($notes->includedPrint) && $notes->includedPrint->checked == true)
                ...
                <tr>
                    ...
                    <td id="totalPrint" class="text-sm text-black border border-black text-right px-2">
            ...
            @if (isset($notes->includedInstall) && $notes->includedInstall->checked == true)
                ...
                <tr>
                    ...
                    <td id="totalInstall" class="text-sm text-black border border-black text-right px-2">
        ...
        <tr>
            @if ($category == 'Signage')
                <td class="border border-black px-2 text-right text-sm text-black font-semibold" colspan="7">
                    SUB TOTAL
                </td>
            @else
                <td class="border border-black px-2 text-right text-sm text-black font-semibold" colspan="6">
                    SUB TOTAL
                </td>
            @endif
        ...
        <tr>
            @if ($category == 'Signage')
                <td class="border border-black px-2 text-right text-sm text-black font-semibold" colspan="7">PPN
                    {{-- {{ $sale->ppn }}% --}}
                </td>
            @else
                <td class="border border-black px-2 text-right text-sm text-black font-semibold" colspan="6">PPN
                    {{-- {{ $sale->ppn }}% --}}
                </td>
            @endif
            <td class="text-sm text-black border border-black text-right px-2 font-semibold">
