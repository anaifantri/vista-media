    ...
    <div class="xl:flex xl:justify-center">
        <div class="mt-10 z-0 mb-8">
            <div class="flex p-1 w-full">
                <h1 class="index-h1">Daftar Lokasi Signage</h1>
                @canany(['isAdmin', 'isMedia', 'isMarketing'])
                    <div class="border-b">
                        <a href="/dashboard/media/signages/create" class="index-link btn-primary">
                            ...
                            <span class="mx-1 hidden sm:flex">Tambah Signage</span>
                        </a>
                    </div>
                @endcanany
            </div>
            <div class="">
                <form action="/dashboard/media/signages/">
                    <div class="flex mt-1 ml-2">
                        ...
                        <div class="w-full md:w-36 ml-2">
                            ...
                            <select class="w-full border rounded-lg text-base text-teal-900 outline-none" name="condition"
                                ...
                                <?php $dataCondition = ['All', 'Terbangun', 'Rencana', 'Tidak Ada']; ?>
                                @for ($i = 0; $i < count($dataCondition); $i++)
                                    @if (request('condition') == $dataCondition[$i])
                                        <option value="{{ $dataCondition[$i] }}" selected> <?php echo $dataCondition[$i]; ?> </option>
                                    @else
                                        <option value="{{ $dataCondition[$i] }}"> <?php echo $dataCondition[$i]; ?> </option>
                                    @endif
                                @endfor
                            </select>
                        </div>
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="bg-teal-100 h-10">
                            ...
                            <th class="text-teal-700 border text-sm text-center w-28">Jenis</th>
                            <th class="text-teal-700 border text-sm text-center w-32">Size - V/H</th>
                            <th class="text-teal-700 border text-sm text-center w-16">Jumlah</th>
                            <th class="text-teal-700 border text-sm text-center w-28">Kondisi</th>
                            <th class="text-teal-700 border text-sm text-center w-32">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            $number = 1 + ($signages->currentPage() - 1) * $signages->perPage();
                        @endphp
                        @foreach ($signages as $signage)
                            @if ($number % 2 != 0)
                                <tr>
                                    ...
                                    <td class="text-teal-700 border text-sm text-center">{{ $signage->qty }}</td>
                                    <td class="text-teal-700 border text-sm text-center">{{ $signage->condition }}
                                    </td>
                                    <td class="text-teal-700 border text-sm text-center">
                                        <div class="flex justify-center items-center">
                                            <a href="/dashboard/media/signages/{{ $signage->id }}"
                            ...
                            @else
                                <tr class="index-tr items-start h-max bg-teal-50">
                                    ...
                                    <td class="index-td text-sm w-[84px] 2xl:w-32">{{ $signage->code }} -
                                        {{ $signage->city->code }}
                                    </td>
                                    <td class="flex justify-start items-center text-sm w-56 2xl:w-80">
                                        {{ $signage->address }}
                                    </td>
                                    <td class="index-td text-sm w-12 2xl:w-16">{{ $signage->city->code }}</td>
                                    <td class="index-td text-sm w-24 2xl:w-28">{{ $signage->signage_category->name }}</td>
                                    <td class="index-td text-sm w-[88px] 2xl:w-24">{{ $signage->size->size }}
                                        -
                                        @if ($signage->size->orientation == 'Vertikal')
                                            V
                                        @elseif ($signage->size->orientation == 'Horizontal')
                                            H
                                        @endif
                                    </td>
                                    <td class="index-td text-sm w-12 2xl:w-16">{{ $signage->qty }}</td>
                                    <td class="index-td text-sm w-[100px] 2xl:w-28">{{ $signage->condition }}
                                    </td>
                                    <td class="flex items-center justify-center text-sm w-36">Rp.
                                        {{ number_format($signage->exclusive_price) }},-</td>
                                    <td class="flex items-center justify-center text-sm w-36">Rp.
                                        {{ number_format($signage->sharing_price) }},-</td>
                                    <td class="flex items-center justify-center text-sm w-36">Rp.
                                        {{ number_format($signage->price) }},-</td>
            ...
            <div class="flex justify-center text-teal-900">
                {{-- {{ $signages->links() }} --}}
                {!! $signages->appends(Request::query())->render() !!}
            </div>
        </div>
    </div>
    <script>
