    ...
    <div class="mt-10 z-0">
        <div class="flex justify-center w-full">
            <div class="w-full md:w-[575px] p-2">
                <div class="flex">
                    ...
                    @canany(['isAdmin', 'isMedia'])
                        <div class="flex border-b">
                            <a href="/dashboard/media/sizes/create" class="index-link btn-primary"><span></span>
                ...
                <form class="mt-2" action="/dashboard/media/sizes/">
                    <div class="flex">
        ...
        <!-- View Size start -->
        <div class="flex md:justify-center p-2 w-full overflow-x-scroll xl:overflow-x-visible z-0">
            <table class="table-auto md:w-[560px] mb-6">
                <thead class="bg-slate-100">
                    <tr class="index-tr items-center h-10 bg-teal-100 border-t">
                        <th class="index-td w-8">No.</th>
                        <th class="index-td w-28">
                            @sortablelink('size', 'Ukuran')
                            <svg class="fill-current w-3 ml-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M12 0l8 10h-16l8-10zm8 14h-16l8 10 8-10z" />
                            </svg>
                        </th>
                        <th class="index-td w-10">
                            @sortablelink('side', 'Sisi')
                            <svg class="fill-current w-3 ml-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M12 0l8 10h-16l8-10zm8 14h-16l8 10 8-10z" />
                            </svg>
                        </th>
                        <th class="index-td w-36">
                            @sortablelink('orientation', 'Orientasi')
                            <svg class="fill-current w-3 ml-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M12 0l8 10h-16l8-10zm8 14h-16l8 10 8-10z" />
                            </svg>
                        </th>
                ...
                <tbody>
                    @php
                        $number = 1 + ($sizes->currentPage() - 1) * $sizes->perPage();
                    @endphp
                    @foreach ($sizes as $size)
                        <tr class="index-tr">
                            <td class="index-td w-8">{{ $number++ }}</td>
                            <td class="index-td w-28">{{ $size->size }}</td>
                            <td class="index-td w-10">{{ $size->side }}</td>
                            <td class="index-td w-36">{{ $size->orientation }}</td>
                            <td class="index-td w-24">{{ $size->user->name }}</td>
                            <td class="index-td w-36">
                                <a href="/dashboard/media/sizes/{{ $size->id }}"
                                ...
                                <a href="/dashboard/media/sizes/{{ $size->id }}/edit"
                                ...
                                @can('isAdmin')
                                    <form action="/dashboard/media/sizes/{{ $size->id }}" method="post"
                                        class="d-inline m-1">
                                        @method('delete')
                                        @csrf
