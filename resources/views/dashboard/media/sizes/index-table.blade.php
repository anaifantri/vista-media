    ...
    <div class="index-container">
        ...
        @canany(['isAdmin', 'isMarketing'])
            <div class="index-btnAdd">
                <a href="/dashboard/media/sizes/create" class="index-link btn-primary">
                    ...
                    <span class="mx-1">Tambah Ukuran</span>
                </a>
            </div>
        @endcanany
            ...
            <table class="table-auto">
                <thead>
                    <tr class="index-tr bg-slate-50">
                        ...
                        <th class="index-td text-sm w-28">Ukuran</th>
                        <th class="index-td text-sm w-16">Sisi</th>
                        <th class="index-td text-sm w-16">Orientasi</th>
                ...
                <tbody>
                    @foreach ($sizes as $size)
                        <tr class="index-tr">
                            ...
                            <td class="index-td text-sm w-28">{{ $size->size }}</td>
                            <td class="index-td text-sm w-16">{{ $size->side }}</td>
                            <td class="index-td text-sm w-16">{{ $size->orientation }}</td>
                            <td class="index-td text-sm w-32">{{ $size->user->name }}</td>
                            <td class="index-td text-sm w-36">
                                ...
                                @canany(['isAdmin', 'isMarketing'])
                                    ...
                                    <form action="/dashboard/media/sizes/{{ $size->id }}" method="post" class="flex m-1">
                                        @method('delete')
                                        @csrf
                                        <button
                                            class="index-link text-white w-8 h-5 rounded bg-red-600 hover:bg-red-700 drop-shadow-md"
                                            onclick="return confirm('Apakah anda yakin ingin menghapus ukuran {{ $size->size }} - {{ $size->side }} sisi - {{ $size->orientation }} ?')">
                                            <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd"
                                                stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
