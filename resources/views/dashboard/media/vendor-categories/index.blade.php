...
@section('container')
    <!-- Container Index Vendor Category start -->
    <div class="mt-10 z-0">
        <div class="flex justify-center w-full">
            <div class="w-[800px] p-2">
                <div class="flex">
                    <!-- Title Vendor Category start -->
                    <h1 class="index-h1"> DAFTAR KATAGORI VENDOR</h1>
                    <!-- Title Vendor Category end -->
                    <!-- Button Create New Vendor Category start -->
                    @canany(['isAdmin', 'isMedia'])
                        <div class="flex border-b">
                            <a href="/dashboard/media/vendor-categories/create" class="index-link btn-primary"><span></span>
                                ...
                                <span class="mx-1 hidden sm:flex"> Tambah Katagori Vendor</span>
                            </a>
                        </div>
                    @endcanany
                </div>
                <!-- Button Create New Vendor Category end -->
                <!-- Alert Success Create New Vendor Category start -->
                <form class="mt-2" action="/dashboard/media/vendor-categories/">
                    <div class="flex">
        ...
        <!-- Alert Success Create New Vendor Category end -->
        <!-- View Vendor Category start -->
        <div class="flex justify-center w-full">
            <div class="w-[800px]">
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        @php
                            $number = 1 + ($vendor_categories->currentPage() - 1) * $vendor_categories->perPage();
                        @endphp
                        @foreach ($vendor_categories as $vendor_category)
                            <tr>
                                <td class="text-teal-700 border text-xs text-center">{{ $number++ }}</td>
                                <td class="text-teal-700 border text-xs text-center">{{ $vendor_category->name }}</td>
                                <td class="px-2 text-teal-700 border text-xs">{{ $vendor_category->description }}
                                </td>
                                <td class="text-teal-700 border text-xs text-center">
                                    <div class="flex justify-center items-center">
                                        ...
                                        @can('isAdmin')
                                            <form action="/dashboard/media/vendor-categories/{{ $vendor_category->id }}"
                                                method="post" class="d-inline m-1">
                                                @method('delete')
                                                @csrf
                                                <button
                                                    class="index-link text-white w-7 h-5 bg-red-500 rounded-md hover:bg-red-600"
                                                    onclick="return confirm('Apakah anda yakin ingin menghapus data katagori vendor dengan nama {{ $vendor_category->name }} ?')">
    ...
    <!-- View Vendor Category end -->
    <!-- Container Index Vendor Category end -->
@endsection
