        ...
        <div class="mt-10 z-0 mb-8">
            <div class="flex p-1 w-full">
                ...
                @canany(['isAdmin', 'isMedia', 'isMarketing'])
                    <div class="border-b">
                        <a href="/dashboard/media/videotrons/create" class="index-link btn-primary">
                            ...
                            <span class="mx-1 hidden sm:flex">Tambah Videotron</span>
                        </a>
                    </div>
                @endcanany
            </div>
            <div class="">
                <form action="/dashboard/media/videotrons/">
            ...
            <div class="w-[1200px]">
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        @php
                            $number = 1 + ($videotrons->currentPage() - 1) * $videotrons->perPage();
                        @endphp
                        @foreach ($videotrons as $videotron)
                            @if ($number % 2 != 0)
                                <tr>
                                    ...
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->led->pixel_pitch }}
                                    </td>
                                    ...
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->condition }}</td>
                                    <td class="text-teal-700 border text-sm text-center">
                                        <div class="flex justify-center items-center">
                                            <a href="/dashboard/media/videotrons/{{ $videotron->id }}"
                                            ...
                                            @canany(['isAdmin', 'isMedia', 'isMarketing'])
                                                ...
                                                <form action="/dashboard/media/videotrons/{{ $videotron->id }}"
                                                    method="post" class="flex m-1">
                                                    @method('delete')
                                                    @csrf
                                                    <button
                                                        class="index-link text-white w-8 h-5 rounded bg-red-600 hover:bg-red-700 drop-shadow-md"
                                                        onclick="return confirm('Apakah anda yakin ingin menghapus {videotron dengan kode {{ $videotron->code }} ?')">
                                                        <svg class="fill-current w-[18px] 2xl:w-5" clip-rule="evenodd"
                                                            fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2"
                                                            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                            <path
                            ...
                            @else
                                <tr class=" bg-teal-50">
                                    ...
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->city->city }}</td>
                                    <td class="text-teal-700 border text-sm text-center">
                                        {{ $videotron->led->pixel_pitch }}</td>
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->size->size }}
                                        -
                                        @if ($videotron->size->orientation == 'Vertikal')
                                            V
                                        @elseif ($videotron->size->orientation == 'Horizontal')
                                            H
                                        @endif
                                    </td>
                                    {{-- <td class="text-teal-700 border text-sm text-center">{{ $videotron->slots }}</td>
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->duration }}</td>
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->start_at }}</td>
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->end_at }}</td> --}}
                                    <td class="text-teal-700 border text-sm text-center">{{ $videotron->condition }}</td>
                                    <td class="text-teal-700 border text-sm text-center">
                                        <div class="flex justify-center items-center">
                                            <a href="/dashboard/media/videotrons/{{ $videotron->id }}"
            ...
            <div class="flex justify-center text-teal-900">
                {{-- {{ $videotrons->links() }} --}}
                {!! $videotrons->appends(Request::query())->render() !!}
            </div>
        </div>
    </div>
    <script>
