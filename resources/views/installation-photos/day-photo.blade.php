<div class="w-[580px] border rounded-lg">
    <label class="flex w-full justify-center text-stone-100 test-sm font-semibold">Foto Siang</label>
    <div class="w-[560px] p-2">
        <div class="flex w-full justify-center">
            <a class="flex justify-center items-center w-44 btn-primary-small" title="Tambah Foto"
                href="/installation-photos/create/{{ $install_order->id }}/day">
        ...
        <figure id="figureDayPhoto" class="flex w-[560px] justify-center overflow-x-auto bg-stone-800 rounded-lg mt-2">
            @foreach ($day_photos as $photo)
                @if (count($day_photos) > 2)
                    @if ($loop->iteration - 1 == intdiv(count($day_photos), 2))
                        <img id="day{{ $loop->iteration - 1 }}" class="documentation-photo-active"
                            src="{{ asset('storage/' . $photo->image) }}" alt=""
                            onclick="figureDayAction(this)">
                    @else
            ...
            @if (count($installation_photos) > 0)
                <div class="absolute inset-y-0 left-0 w-7 h-12 m-auto">
                    <button
                        ...
                        type="button" onclick="buttonDayPrev()">
