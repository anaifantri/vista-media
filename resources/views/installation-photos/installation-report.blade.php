...
@section('container')
    @php
        ...
        if (fmod(count($install_orders), 30) == 0) {
            $totalPages = count($install_orders) / 30;
        } else {
            $totalPages = (count($install_orders) - fmod(count($install_orders), 30)) / 30 + 1;
        }
    @endphp
    <!-- Container start -->
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            ...
            <div>
                <!-- Form search start -->
                <form action="/installation-photos/report/{{ $company->id }}">
                    <div class="flex mt-1">
                        <div class="w-36">
                                        ...
                                        <thead>
                                            <tr class="bg-stone-400">
                                                <th class="text-stone-900 border border-stone-900 text-xs w-8 text-center">
                                                    No.</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-36">
                                                    No. Penj.</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-40">
                                                    No. SPK</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-24">
                                                    Tgl. Tayang</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-24">
                                                    Klien</th>
                                                <th class="text-stone-900 border border-stone-900 text-xs text-center">
                                                    Tema/Design</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs w-[72px] text-center">
                                                    Kode</th>
                                                <th class="text-stone-900 border border-stone-900 text-xs text-center">
                                                    Lokasi
                                                </th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-[88px]">
                                                    Foto Siang
                                                </th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-xs text-center w-[88px]">
                                            ...
                                            @foreach ($install_orders as $order)
                                                @if ($loop->iteration > $i * 30 && $loop->iteration < ($i + 1) * 30 + 1)
                                                    @php
                                                        ...
                                                        $night_photos = $order->installation_photos->where(
                                                            'type',
                                                            'night',
                                                        );
    ...
    <input id="saveName" type="text" value="LIST PEMASANGAN GAMBAR" hidden>
    <!-- Container end -->
    <!-- Script start -->
        ...
        $(document).ready(function() {
            $('#btnExportExcel').on('click', function() {
                $('#exportExcelTable').table2excel({
                    filename: "List Pemasangan Gambar.xls"
                });
            });
        });
    </script>
