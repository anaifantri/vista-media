...
@section('container')
        ...
        <div>
            <div class="mt-10">
                <div class="flex w-[1100px] border-b">
                    <h1 class="flex text-xl text-cyan-800 font-bold tracking-wider w-[600px] py-1">Pilih Lokasi</h1>
                    <div class="flex justify-end w-full">
                        ...
                        <a class="flex justify-center items-center ml-1 btn-danger" href="/marketing/install-orders">
                ...
                <form action="/install-orders/select-locations">
                    ...
                    <div class="flex mt-1 ml-2">
                        @if (request('orderType'))
                            @if (request('orderType') == 'sales')
                                <input class="outline-none" name="orderType" type="radio" onclick="submit()"
                                    value="sales" checked>
                                <label class="text-sm text-teal-900 ml-2" for="sales">Pemasangan Berbayar</label>
                            @else
                                <input class="outline-none" name="orderType" type="radio" onclick="submit()"
                                    value="sales">
                                <label class="text-sm text-teal-900 ml-2" for="sales">Pemasangan Berbayar</label>
                            ...
                            @if (request('orderType') == 'free')
                                <input class="outline-none ml-4" name="orderType" type="radio" onclick="submit()"
                                    value="free" checked>
                                <label class="text-sm text-teal-900 ml-2" for="free">Free Pemasangan Klien</label>
                            @else
                                <input class="outline-none ml-4" name="orderType" type="radio" onclick="submit()"
                                    value="free">
                                <label class="text-sm text-teal-900 ml-2" for="free">Free Pemasangan Klien</label>
                            ...
                            @if (request('orderType') == 'locations')
                                ...
                                <label class="text-sm text-teal-900 ml-2" for="locations">Free Pemasangan Lain-Lain</label>
                            @else
                                <input class="outline-none  ml-4" name="orderType" type="radio" onclick="submit()"
                                    value="locations">
                                <label class="text-sm text-teal-900 ml-2" for="locations">Free Pemasangan Lain-Lain</label>
                    ...
                    <div class="flex mt-1 ml-2">
                        <div class="w-36">
                            <span class="text-sm text-teal-900">Area</span>
                            @if (request('area'))
                                <select class="w-full border rounded-lg text-sm text-teal-900 outline-none" name="area"
                        ...
                        <div class="w-36 ml-2">
                            <span class="text-sm text-teal-900">Kota</span>
                            @if (request('area'))
                                @if (request('area') != 'All')
                                    <select id="city"
                                        class="w-full border rounded-lg text-sm text-teal-900 outline-none" name="city"
                                        onchange="submit()">
                                        <option value="All">All</option>
                        ...
                        <div class="ml-2 w-36">
                            <span class="text-sm text-teal-900">Katagori</span>
                            <select class="w-full border rounded-lg text-sm text-teal-900 outline-none"
                    ...
                    <div class="flex mt-2">
                        <div class="flex">
                            <input id="search" name="search"
                                class="flex border rounded-l-lg ml-2 p-1 outline-none text-sm text-teal-900"
            ...
            <div class="w-[1100px] mt-4">
                @if (request('orderType'))
                    ...
                    @elseif (request('orderType') == 'sales')
                        {{-- @include('install-orders.sales-table') --}}
                    @elseif (request('orderType') == 'free')
                        @include('install-orders.free-table')
                    @endif
                @else
                    {{-- @include('install-orders.sales-table') --}}
                @endif
    ...
    <script src="/js/selectinstalllocation.js"></script>
@endsection
