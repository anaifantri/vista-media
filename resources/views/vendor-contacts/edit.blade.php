...
@section('container')
    ...
    <div class="w-full flex justify-center mt-4" id="editContact" name="editContact">
        <div class="w-[600px] h-[400px] bg-white rounded-xl border flex mt-8">
            ...
            <form class="flex" method="post" action="/vendor-contacts/{{ $contact->id }}" enctype="multipart/form-data">
                @method('put')
                @csrf
                ...
                <div class="mt-12 w-[290px] bg-white p-1">
                    <div class="mt-1"><label class="text-sm text-teal-700">Nama Vendor</label>
                        <input id="vendor_name" name="vendor_name" title="Terisi otomatis"
                            ...
                            type="text" value="{{ $contact->vendor->name }}" required readonly>
                        <input id="vendor_id" name="vendor_id" value="{{ $contact->vendor_id }}" type="hidden">
                    </div>
