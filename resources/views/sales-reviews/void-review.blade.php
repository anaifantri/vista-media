        ...
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            ...
            <div class="flex justify-center mt-2">
                <div class="mt-1 rounded-lg w-[440px] p-2 bg-white">
                    ...
                    <div class="mt-4">
                        <label class="flex text-sm font-semibold underline text-stone-900">Alasan pembatalan</label>
                        <label class="flex text-sm w-[400px]">{{ $void_sale->note }}</label>
                    </div>
                    <div class="mt-4">
                        <label class="flex text-sm font-semibold text-stone-900">Tgl. pembatalan :
                            {{ date('d', strtotime($void_sale->created_at)) }}
                            {{ $bulan[(int) date('m', strtotime($void_sale->created_at))] }}
                            {{ date('Y', strtotime($void_sale->created_at)) }}</label>
                    </div>
                </div>
            ...
            <div class="flex justify-center items-center mt-2 border rounded-lg p-2">
                <a class="flex justify-center items-center btn-danger"
                    href="/sales-review/{{ $company->id }}?month={{ (int) date('m', strtotime($void_sale->created_at)) }}&year={{ date('Y', strtotime($void_sale->created_at)) }}">
                ...
                @if ($reviewed == false)
                    <form class="d-inline m-1" method="post" action="/void-review" enctype="multipart/form-data">
                        @csrf
                        <input type="text" name="void_sale_id" value="{{ $void_sale->id }}" hidden>
                        ...
                        <input type="text" name="void_month"
                            value="{{ (int) date('m', strtotime($void_sale->created_at)) }}" hidden>
                        <input type="text" name="void_year"
                            value="{{ date('Y', strtotime($void_sale->created_at)) }}" hidden>
                ...
                @else
                    @if ($ownerReviewed == false)
                        <a href="/void-review/unchecked/{{ $reviewedId }}"
                            class="flex justify-center items-center btn-success mx-2"
                            onclick="return confirm('Apakah anda yakin ingin menghapus / membatalkan konfirmasi pemeriksaan pembatalan penjualan ini?')">
                            <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd"
                                stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
