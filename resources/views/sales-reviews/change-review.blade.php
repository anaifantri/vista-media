        ...
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            ...
            <div class="flex justify-center mt-2">
                <div class="mt-1 rounded-lg w-[440px] p-2 bg-white">
                    ...
                    <div class="mt-4">
                        <label class="flex text-sm font-semibold underline text-stone-900">Alasan perubahan</label>
                        <label class="flex text-sm w-[400px]">{{ $change_sale->note }}</label>
                    </div>
                    <div class="mt-4">
                        <label class="flex text-sm font-semibold text-stone-900">Tgl. perubahan :
                            {{ date('d', strtotime($change_sale->created_at)) }}
                            {{ $bulan[(int) date('m', strtotime($change_sale->created_at))] }}
                            {{ date('Y', strtotime($change_sale->created_at)) }}</label>
                    </div>
                </div>
            ...
            <div class="flex justify-center items-center mt-2 border rounded-lg p-2">
                <a class="flex justify-center items-center btn-danger"
                    href="/sales-review/{{ $company->id }}?month={{ (int) date('m', strtotime($change_sale->created_at)) }}&year={{ date('Y', strtotime($change_sale->created_at)) }}">
                ...
                @if ($reviewed == false)
                    <form class="d-inline m-1" method="post" action="/change-review" enctype="multipart/form-data">
                        @csrf
                        <input type="text" name="change_sale_id" value="{{ $change_sale->id }}" hidden>
                        ...
                        <input type="text" name="change_month"
                            value="{{ (int) date('m', strtotime($change_sale->created_at)) }}" hidden>
                        <input type="text" name="change_year"
                            value="{{ date('Y', strtotime($change_sale->created_at)) }}" hidden>
                ...
                @else
                    @if ($ownerReviewed == false)
                        <a href="/change-review/unchecked/{{ $reviewedId }}"
