    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            <div class="flex justify-center w-full">
                <div class="w-[1000px]">
                    <div class="flex border-b">
                        <!-- Title start -->
                        <h1 class="index-h1"> DAFTAR BAHAN CETAK</h1>
                        <!-- Title end -->
                        <!-- Button start -->
                        @canany(['isAdmin', 'isMarketing'])
                            @can('isMarketingSetting')
                                @can('isMarketingCreate')
                                    <div class="flex">
                                        <a href="/marketing/printing-products/create" class="index-link btn-primary"><span></span>
                                            ...
                                            <span class="mx-1 hidden sm:flex"> Tambah Bahan Cetak</span>
                                        </a>
                                    </div>
                                @endcan
                            @endcan
                    ...
                    <!-- Button Printing Product end -->
                    <!-- Alert start -->
                    <form class="mt-2" action="/marketing/printing-products/">
                        <div class="flex">
                        ...
                        <thead>
                            <tr class="bg-stone-400 h-10">
                                ...
                                <th class="text-stone-900 border border-stone-900 text-xs text-center w-32">
                                    <button class="flex justify-center items-center w-32">@sortablelink('name', 'Nama Bahan')
                                        <svg class="fill-current w-3 ml-1" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24">
                                            <path d="M12 0l8 10h-16l8-10zm8 14h-16l8 10 8-10z" />
                                        </svg>
                                ...
                                <th class="text-stone-900 border border-stone-900 text-xs text-center w-20">Type</th>
                                <th class="text-stone-900 border border-stone-900 text-xs text-center">Deskripsi</th>
                                <th class="text-stone-900 border border-stone-900 text-xs text-center w-28">Harga (Rp.)</th>
                                <th class="text-stone-900 border border-stone-900 text-xs text-center w-24">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-stone-300">
                            @php
                                $number = 1 + ($printing_products->currentPage() - 1) * $printing_products->perPage();
                            @endphp
                            @foreach ($printing_products as $printing_product)
                                <tr>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        {{ $number++ }}</td>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        {{ $printing_product->code }}</td>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        {{ $printing_product->name }}</td>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        {{ $printing_product->type }}</td>
                                    <td class="px-2 text-stone-900 border border-stone-900 text-xs">
                                        {{ $printing_product->description }}</td>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        {{ number_format($printing_product->price) }}</td>
                                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                        <div class="flex justify-center items-center">
                                            <a href="/marketing/printing-products/{{ $printing_product->id }}"
                                                    ...
                                                    @can('isMarketingDelete')
                                                        <form action="/marketing/printing-products/{{ $printing_product->id }}"
                                                            ...
                                                            <button
                                                                ...
                                                                onclick="return confirm('Apakah anda yakin ingin menghapus data bahan cetak dengan nama {{ $printing_product->name }} ?')">
            ...
            <div class="flex justify-center text-stone-100">
                {!! $printing_products->appends(Request::query())->render('dashboard.layouts.pagination') !!}
            </div>
        </div>
    </div>
    <!-- Container end -->
