...
@section('container')
    ...
    @endphp
    <!-- Form Create start -->
    <form id="formCreate" action="/workshop/complaints/{{ $complaint->id }}" method="post" enctype="multipart/form-data">
        @method('put')
        @csrf
        <input type="text" name="updated_by" id="updated_by" value="{{ json_encode($updated_by) }}" hidden>
        <input type="text" name="old_images" id="updated_by" value="{{ $complaint->images }}" hidden>
        @if (old('descriptions'))
        ...
        <div class="flex justify-center pl-14 py-10 bg-stone-800">
            <div class="p-4 w-[1000px] border rounded-lg bg-stone-700">
                <div class="flex items-center justify-center border-b p-1">
                    <h4 class="text-xl font-semibold tracking-wider text-stone-100 w-[950px]">EDIT DATA KOMPLAIN DARI
                        KLIEN</h4>
                    ...
                    <div class="border rounded-lg p-2 bg-stone-300">
                        <div>
                            <div>
                                ...
                                <input
                                    class="flex w-[150px] text-sm font-semibold text-stone-900 border rounded-lg p-1 outline-none"
                                    value="{{ $complaint->number }}" type="text" readonly>
                            </div>
                            <div>
                                <span class="text-sm text-stone-900">Tgl. Komplain</span>
                                @if (old('complaint_date'))
                                ...
                                @else
                                    <input name="complaint_date"
                                        class="flex w-[150px] text-sm font-semibold text-stone-900 border rounded-lg p-1 outline-none @error('complaint_date') is-invalid @enderror"
                                        value="{{ $complaint->complaint_date }}" type="date" required>
                                @endif
                            </div>
                            ...
                            <div class="mt-2">
                                ...
                                @else
                                    <textarea
                                        class="flex w-[450px] text-sm font-semibold text-stone-900 border rounded-lg p-1 outline-none @error('notes') is-invalid @enderror"
                                        name="notes" rows="5" placeholder="Input Catatan" required>{{ $complaint->notes }}</textarea>
                                @endif
                            </div>
                            @error('notes')
                                <div class="text-red-600 flex mx-2">
                    ...
                    <div class="flex items-center justify-center border-b p-1">
                        ...
                        <div class="flex justify-end w-full">
                            <button id="btnSave" name="btnSave"
                                class="flex justify-center items-center ml-1 btn-primary" type="button"
                                onclick="changePhoto()">Ganti Photo</button>
                        </div>
                    </div>
