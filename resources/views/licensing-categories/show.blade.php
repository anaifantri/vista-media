...
@section('container')
    <?php
    $createdDate = strtotime($licensing_category->created_at);
    $updatedDate = strtotime($licensing_category->updated_at);
    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="p-4 w-[350px] h-[500px] border rounded-lg bg-stone-400">
            <div class="flex justify-center items-center mb-3 w-full border-b">
                <h4 class="text-xl font-semibold tracking-wider text-stone-900">Detail Katagori
            ...
            <div>
                ...
                <div class="flex justify-center mt-2 w-full">
                    <div class="border-b w-[280px]">
                        ...
                        <label class="flex text-semibold">{{ $licensing_category->name }}</label>
                    </div>
                </div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="w-[280px]">
                        <label class="text-sm text-stone-900">Deskripsi</label>
                        <textarea class="flex text-semibold w-[280px] border rounded-lg p-1 outline-teal-300" rows="4" readonly>{{ $licensing_category->description }}</textarea>
                    </div>
                </div>
                <div class="flex justify-center mt-2 w-full">
                    <div class="border-b w-[280px]">
                ...
                @if ($licensing_category->created_at != $licensing_category->updated_at)
                    <div class="flex justify-center mt-2 w-full">
                        <div class="border-b w-[280px]">
                            <label class="flex text-sm text-stone-900">Diupdate Oleh</label>
                            <label class="flex text-semibold">{{ $licensing_category->user->name }}</label>
                        </div>
                    </div>
                @else
                    <div class="flex justify-center mt-2 w-full">
                ...
                <div class="flex justify-center mt-2">
                    <a href="/media/licensing-categories" class="flex items-center justify-center btn-primary mx-1">
                    ...
                    @canany(['isAdmin', 'isMedia'])
                        @can('isLegal')
                            @can('isMediaEdit')
                                <a href="/media/licensing-categories/{{ $licensing_category->id }}/edit"
                    ...
                    @canany(['isAdmin', 'isMedia'])
                        @can('isLegal')
                            @can('isMediaDelete')
                                <form action="/media/licensing-categories/{{ $licensing_category->id }}" method="post"
                                    class="d-inline">
                                    @method('delete')
                                    @csrf
                                    <button class="flex items-center justify-center btn-danger mx-1"
                                        onclick="return confirm('Apakah anda yakin ingin menghapus data katagori dengan nama {{ $licensing_category->name }} ?')">
                                        <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                            stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
