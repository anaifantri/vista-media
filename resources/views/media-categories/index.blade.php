    ...
    <div class="mt-10 z-0">
        <div class="flex justify-center w-full">
            <div class="w-[1000px]">
                <div class="flex border-b">
                    ...
                    @canany(['isAdmin', 'isMedia'])
                        @can('isMediaSetting')
                            @can('isMediaCreate')
                                <div class="flex">
                                    <a href="/media/media-categories/create" class="index-link btn-primary"><span></span>
                ...
                <!-- Form Search start -->
                <form class="mt-2" action="/media/media-categories/">
                    <div class="flex">
        ...
        <div class="flex justify-center w-full mt-2">
            <div class="w-[1000px]">
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        @php
                            $number = 1 + ($media_categories->currentPage() - 1) * $media_categories->perPage();
                        @endphp
                            ...
                            <tr>
                                ...
                                <td class="text-teal-700 border text-sm text-center">
                                    <div class="flex justify-center items-center">
                                        <a href="/media/media-categories/{{ $category->id }}"
                                        ...
                                        @canany(['isAdmin', 'isMedia'])
                                            @can('isMediaSetting')
                                                @can('isMediaEdit')
                                                    <a href="/media/media-categories/{{ $category->id }}/edit"
                                        ...
                                        @canany(['isAdmin', 'isMedia'])
                                            @can('isMediaSetting')
                                                @can('isMediaDelete')
                                                    <form action="/media/media-categories/{{ $category->id }}" method="post"
                                                        class="d-inline m-1">
                                                        @method('delete')
                                                        @csrf
                                                        <button
        ...
        <div class="flex justify-center text-teal-900">
            {{-- {{ $media_categories->links() }} --}}
            {!! $media_categories->appends(Request::query())->render('dashboard.layouts.pagination') !!}
        </div>
    </div>
    <!-- View end -->
    <!-- Container end -->
