    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            <div class="flex p-1 w-full border-b">
                <!-- Title start -->
                <h1 class="index-h1">Daftar Penurunan Materi Videotron</h1>
                <!-- Title end -->
                <div class="flex items-center w-full justify-end">
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isContent')
                            @can('isWorkshopCreate')
                                <a href="/workshop/takeout-contents/create" title="Tambah Data Takeout"
            ...
            <div>
                <!-- Form search start -->
                <form action="/workshop/takeout-contents/">
                    <div class="flex mt-1">
                        <div class="w-36">
            ...
            <div class="w-[1550px] mt-2">
                <table class="table-auto w-full">
                    ...
                    <tbody class="bg-stone-200">
                        @php
                            $number = 1 + ($take_out_contents->currentPage() - 1) * $take_out_contents->perPage();
                        @endphp
                        @foreach ($take_out_contents as $content)
                            <tr>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    {{ $number++ }}
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    {{ date('d', strtotime($content->publish_content->publish_date)) }}-{{ $bulan[(int) date('m', strtotime($content->publish_content->publish_date))] }}-{{ date('Y', strtotime($content->publish_content->publish_date)) }}
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    {{ date('d', strtotime($content->take_out_date)) }}-{{ $bulan[(int) date('m', strtotime($content->take_out_date))] }}-{{ date('Y', strtotime($content->take_out_date)) }}
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                ...
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    @if ($content->publish_content->status == 'Free')
                                        Gratis
                                    @else
                                        Berbayar
                                    @endif
                                ...
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    {{ $content->publish_content->theme }}
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    <div class="flex justify-center items-center">
                                        <a href="/workshop/takeout-contents/{{ $content->id }}"
                                        ...
                                        @canany(['isAdmin', 'isMedia', 'isMarketing', 'isWorkshop', 'isOwner',
                                            ...
                                            @can('isContent')
                                                @can('isWorkshopEdit')
                                                    <a href="/workshop/takeout-contents/{{ $content->id }}/edit"
            ...
            <div class="flex justify-center text-stone-100 mt-2">
                {!! $take_out_contents->appends(Request::query())->render('dashboard.layouts.pagination') !!}
            </div>
            <!-- Pagination end -->
        </div>
    </div>
