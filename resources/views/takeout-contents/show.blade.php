        ...
        <div class="p-4 w-[1000px] border rounded-lg bg-stone-700">
            <div class="flex items-center justify-center border-b p-1">
                ...
                <div class="flex justify-end w-full">
                    <a href="/workshop/takeout-contents" class="flex items-center justify-center btn-primary mx-1">
                    ...
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isContent')
                            @can('isWorkshopEdit')
                                <a href="/workshop/takeout-contents/{{ $take_out_content->id }}/edit"
                    ...
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isContent')
                            @can('isWorkshopDelete')
                                <form action="/workshop/takeout-contents/{{ $take_out_content->id }}" method="post" class="d-inline">
                                    @method('delete')
                                    @csrf
                                    <button class="flex items-center justify-center btn-danger mx-1"
                                        onclick="return confirm('Apakah anda yakin ingin menghapus data takeout materi videotron ini..?')">
                                        <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                            stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                ...
                <div class="border rounded-lg p-2 bg-stone-300">
                    ...
                    <div>
                        ...
                        <label
                            class="flex w-[450px] text-sm font-semibold text-stone-900 border rounded-lg border-stone-900 px-2 py-1">
                            {{ date('d', strtotime($take_out_content->take_out_date)) }}-{{ $bulan_full[(int) date('m', strtotime($take_out_content->take_out_date))] }}-{{ date('Y', strtotime($take_out_content->take_out_date)) }}
                        </label>
                    </div>
                    <div class="mt-2">
