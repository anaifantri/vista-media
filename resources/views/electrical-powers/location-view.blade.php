        ...
        <tbody class="bg-stone-200">
            ...
            @foreach ($locations as $location)
                <tr>
                    ...
                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                        @if (count($location->electrical_powers) > 0)
                            {{ $location->electrical_powers[0]->type }}
                        @else
                            -
                        @endif
                    </td>
                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                        @if (count($location->electrical_powers) > 0)
                            {{ $location->electrical_powers[0]->id_number }}
                        @else
                            -
                        @endif
                    </td>
                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                        @if (count($location->electrical_powers) > 0)
                            {{ $location->electrical_powers[0]->name }}
                        @else
                            -
                        @endif
                    </td>
                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                        @if (count($location->electrical_powers) > 0)
                            {{ $location->electrical_powers[0]->power }}
                        @else
                            -
                        @endif
                    </td>
                    <td class="text-stone-900 border border-stone-900 text-xs text-center">
                        <div class="flex justify-center items-center">
                            @if (count($location->electrical_powers) > 0)
                                <a href="/workshop/electrical-powers/{{ $location->electrical_powers[0]->id }}"
                                ...
                                @canany(['isAdmin', 'isWorkshop', 'isMedia'])
                                    @can('isElectricity')
                                        @can('isWorkshopEdit')
                                            <a href="/workshop/electrical-powers/{{ $location->electrical_powers[0]->id }}/edit"
                                ...
                                @canany(['isAdmin'])
                                    @can('isElectricity')
                                        @can('isWorkshopDelete')
                                            <form action="/workshop/electrical-powers/{{ $location->electrical_powers[0]->id }}"
                                                method="post" class="d-inline m-1">
                                                @method('delete')
                                                @csrf
                                                <button class="index-link text-white w-7 h-5 bg-red-500 rounded-md hover:bg-red-600"
                                                    onclick="return confirm('Apakah anda yakin ingin menghapus daya listrik dengan id {{ $location->electrical_powers[0]->id_number }} ?')">
                                ...
                                @canany(['isAdmin', 'isWorkshop', 'isMedia'])
                                    @can('isElectricity')
                                        @can('isWorkshopCreate')
                                            <a href="/create-electrical-power/{{ $location->id }}" title="Tambah Data Daya Listrik"
                                                class="index-link text-white w-7 h-5 rounded bg-amber-400 hover:bg-amber-500 drop-shadow-md">
                                                <svg class="fill-current w-[18px]" clip-rule="evenodd" fill-rule="evenodd"
                                                    stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24"
                                                    xmlns="http://www.w3.org/2000/svg">
