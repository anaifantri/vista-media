                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="bg-stone-400">
                            ...
                            <th class="text-stone-900 border border-stone-900 text-xs text-center" colspan="3">Data
                                Pengisian</th>
                            <th class="text-stone-900 border border-stone-900 text-xs text-center" colspan="2">Jumlah Kwh
                            </th>
                        ...
                        <tr class="bg-stone-400">
                            ...
                            <th class="text-stone-900 border border-stone-900 text-xs text-center w-16">Sebelum</th>
                            <th class="text-stone-900 border border-stone-900 text-xs text-center w-16">Sesudah</th>
                        </tr>
                    </thead>
                    <tbody class="bg-stone-300">
                        ...
                        @foreach ($locations as $location)
                            @php
                                $last_top_up = $location->electricity_top_ups->last();
                            ...
                            <tr class="bg-stone-100">
                                ...
                                <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                    @if ($last_top_up)
                                        {{ date('d', strtotime($last_top_up->topup_date)) }}
                                        {{ $bulan[(int) date('m', strtotime($last_top_up->topup_date))] }}
                                        {{ date('Y', strtotime($last_top_up->topup_date)) }}
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                    @if ($last_top_up)
                                        {{ number_format($last_top_up->top_up_nominal) }}
                                    @else
                                        -
                                    @endif
                                </td>
                                ...
                                <td class="text-stone-900 border border-stone-900 text-xs text-center">
                                    <div class="flex justify-center items-center">
                                        <a href="/show-electricity-top-up/{{ $location->id }}"
                                            title="Lihat Data Pengisian Pulsa Listrik"
                                        ...
                                        @canany(['isAdmin', 'isWorkshop'])
                                            @can('isElectricity')
                                                @can('isWorkshopCreate')
                                                    <a href="/create-electricity-top-up/{{ $location->id }}"
                                                        title="Tambah Data Pengisian Pulsa Listrik"
