...
@section('container')
    @php
        ...
        if (fmod(count($publish_contents), 30) == 0) {
            $totalPages = count($publish_contents) / 30;
        } else {
            $totalPages = (count($publish_contents) - fmod(count($publish_contents), 30)) / 30 + 1;
        }
    @endphp
    <!-- Container start -->
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            <div class="flex p-1 w-full border-b">
                <!-- Title start -->
                <h1 class="index-h1">List Penayangan Materi Videotron</h1>
                <!-- Title end -->
            ...
            <div>
                <!-- Form search start -->
                <form action="/publish-contents/report">
                    <div class="flex mt-1">
                        <div class="w-36">
                                        ...
                                        <thead>
                                            <tr class="bg-stone-400">
                                                ...
                                                <th class="text-stone-900 border border-stone-900 text-sm text-center">
                                                    Lokasi</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm text-center w-24">
                                                    Ukuran</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm text-center w-56">
                                                    Client</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm text-center w-20">
                                                    Status</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm text-center w-96">
                                                    Tema
                                                </th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm w-28 text-center">
                                                    Tgl.
                                                    Upload</th>
                                                <th
                                                    class="text-stone-900 border border-stone-900 text-sm w-28 text-center">
                                            ...
                                            @foreach ($publish_contents as $content)
                                                @if ($loop->iteration > $i * 30 && $loop->iteration < ($i + 1) * 30 + 1)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td
                                                            class="text-stone-900 border border-stone-900 text-sm text-center">
                                                            {{ $content->location->code }}-{{ $content->location->city->code }}
                                                        </td>
    ...
    <table id="exportExcelTable" class="table-auto w-full" hidden>
        <thead>
            <tr class="bg-stone-400">
                ...
                <th class="text-stone-900 border border-stone-900 text-sm w-28 text-center">
                    Tgl.
                    Upload</th>
                <th class="text-stone-900 border border-stone-900 text-sm w-28 text-center">
                    Tgl. Takeout
    ...
    <input id="saveName" type="text" value="LIST PENAYANGAN MATERI VIDEOTRON" hidden>
    <!-- Container end -->
    <!-- Script start -->
        ...
        $(document).ready(function() {
            $('#btnExportExcel').on('click', function() {
                $('#exportExcelTable').table2excel({
                    filename: "List Penayangan Materi Videotron.xls"
                });
            });
        });
    </script>
