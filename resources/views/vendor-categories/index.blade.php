...
@section('container')
    ...
    <div class="mt-10 z-0">
                    ...
                    <thead>
                        <tr class="bg-teal-100 h-10">
                            ...
                            <th class="text-teal-700 border text-sm text-center w-40">
                                <button class="flex justify-center items-center w-40">@sortablelink('name', 'Katagori')
                                    <svg class="fill-current w-3 ml-1" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 24 24">
                                        <path d="M12 0l8 10h-16l8-10zm8 14h-16l8 10 8-10z" />
                                    </svg>
                    ...
                    <tbody>
                        ...
                        @foreach ($vendor_categories as $vendor_category)
                            <tr>
                                ...
                                <td class="text-teal-700 border text-sm text-center">{{ $vendor_category->code }}</td>
                                <td class="text-teal-700 border text-sm text-center">{{ $vendor_category->name }}</td>
                                <td class="px-2 text-teal-700 border text-sm">{{ $vendor_category->description }}
                                </td>
                                            ...
                                            <form action="/vendor-categories/{{ $vendor_category->id }}" method="post"
                                                ...
                                                @if ($vendor_category->vendors()->exists())
                                                    <button
                                                        ...
                                                        onclick="return confirm('Berelasi dengan {{ count($vendor_category->vendors) }} data vendor, apakah anda yakin ingin menghapus data vendor dengan nama {{ $vendor_category->name }} sekaligus menghapus data-data yang berelasi?')">
                                                ...
                                                @else
                                                    <button
                                                        class="index-link text-white w-7 h-5 bg-red-500 rounded-md hover:bg-red-600"
                                                        onclick="return confirm('Apakah anda yakin ingin menghapus data vendor dengan nama {{ $vendor_category->name }} ?')">
                                                        <svg class="w-4 fill-current" xmlns="http://www.w3.org/2000/svg"
                                                            width="24" height="24" viewBox="0 0 24 24">
                                                            <title>DELETE</title>
                                                            <path
        ...
        <div class="flex justify-center text-teal-900">
            ...
            {!! $vendor_categories->appends(Request::query())->render('dashboard.layouts.pagination') !!}
        </div>
    </div>
