    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            <div class="flex p-1 w-full border-b">
                <!-- Title start -->
                <h1 class="index-h1">Daftar Penyesesaian Komplain Dari Klien</h1>
                <!-- Title end -->
                <div class="flex items-center w-full justify-end">
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isComplaint')
                            @can('isWorkshopCreate')
                                <a href="/workshop/complaint-responses/create" title="Tambah Data" class="index-link btn-primary mx-1">
                    ...
                    @canany(['isAdmin', 'isWorkshop', 'isAccounting', 'isMarketing', 'isMedia'])
                        @can('isComplaint')
                            @can('isWorkshopRead')
                                <button id="btnCreatePdf" class="hidden justify-center items-center mx-1 btn-warning" title="Create PDF"
                                    type="button">
                                ...
                                <button id="btnExportExcel" class="hidden justify-center items-center mx-1 btn-success"
            ...
            <div>
                <!-- Form search start -->
                <form action="/workshop/complaint-responses">
                    <div class="flex mt-1">
                        <div class="w-36">
            ...
            <div class="w-[1550px] mt-2">
                <table class="table-auto w-full">
                    <thead>
                        <tr class="bg-stone-400 h-8">
                            ...
                            <th class="text-stone-900 border border-stone-900 text-sm text-center w-96">
                                Lokasi</th>
                            <th class="text-stone-900 border border-stone-900 text-sm px-1">
                                Permasalahan</th>
                            <th class="text-stone-900 border border-stone-900 text-sm px-1">
                                Penanganan / Perbaikan</th>
                            <th class="text-stone-900 border border-stone-900 text-sm w-32 text-center">Tgl.
                                Penyelesaian</th>
                    ...
                    <tbody class="bg-stone-200">
                        @php
                            $number = 1 + ($complaint_responses->currentPage() - 1) * $complaint_responses->perPage();
                        @endphp
                        @foreach ($complaint_responses as $response)
                            ...
                            <tr>
                                ...
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    <a
                                        href="/workshop/complaints/{{ $response->complaint->id }}">{{ $response->complaint->number }}</a>
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                ...
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    {{ date('d', strtotime($response->response_date)) }}-{{ $bulan[(int) date('m', strtotime($response->response_date))] }}-{{ date('Y', strtotime($response->response_date)) }}
                                </td>
                                <td class="text-stone-900 border border-stone-900 text-sm text-center">
                                    <div class="flex justify-center items-center">
                                        <a href="/workshop/complaint-responses/{{ $response->id }}"
                                        ...
                                        @canany(['isAdmin', 'isMedia', 'isMarketing', 'isWorkshop', 'isOwner',
                                            'isAccounting'])
                                            @can('isComplaint')
                                                @can('isWorkshopEdit')
                                                    <a href="/workshop/complaint-responses/{{ $response->id }}/edit"
                                        ...
                                        @canany(['isAdmin', 'isMedia', 'isMarketing', 'isWorkshop', 'isOwner',
                                            ...
                                            @can('isComplaint')
                                                @can('isWorkshopDelete')
                                                    <form action="/workshop/complaint-responses/{{ $response->id }}" method="post"
                                                        class="d-inline m-1">
                                                        @method('delete')
                                                        @csrf
                                                        <button
                                                            class="index-link text-white w-7 h-5 bg-red-500 rounded-md hover:bg-red-600"
                                                            onclick="return confirm('Apakah anda yakin ingin menghapus data komplain klien ini ?')">
            ...
            <div class="flex justify-center text-stone-100 mt-2">
                {!! $complaint_responses->appends(Request::query())->render('dashboard.layouts.pagination') !!}
            </div>
            <!-- Pagination end -->
        </div>
    </div>
