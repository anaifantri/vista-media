    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="p-4 w-[1000px] border rounded-lg bg-stone-700">
            <div class="flex items-center justify-center border-b p-1">
                <h4 class="text-xl font-semibold tracking-wider text-stone-100 w-[950px]">DETAIL PENANGANAN KOMPLAIN DARI
                    KLIEN</h4>
                <div class="flex justify-end w-full">
                    <a href="/workshop/complaint-responses" class="flex items-center justify-center btn-primary mx-1">
                    ...
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isComplaint')
                            @can('isWorkshopEdit')
                                <a href="/workshop/complaint-responses/{{ $complaint_response->id }}/edit"
                    ...
                    @canany(['isAdmin', 'isWorkshop', 'isMedia', 'isAccounting', 'isMarketing'])
                        @can('isComplaint')
                            @can('isWorkshopDelete')
                                <form action="/workshop/complaint-responses/{{ $complaint_response->id }}" method="post"
                                    class="d-inline">
                                    @method('delete')
                                    @csrf
                                    <button class="flex items-center justify-center btn-danger mx-1"
                                        onclick="return confirm('Apakah anda yakin ingin menghapus data penanganan komplain klien ini..?')">
                                        <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                            stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div class="border rounded-lg p-2 bg-stone-200 w-full">
                    <div>
                        ...
                        <label
                            class="flex w-[150px] text-sm font-semibold bg-neutral-50 text-stone-900 border rounded-lg p-1">
                            {{ date('d', strtotime($complaint_response->response_date)) }}-{{ $bulan_full[(int) date('m', strtotime($complaint_response->response_date))] }}-{{ date('Y', strtotime($complaint_response->response_date)) }}
                        </label>
                    </div>
                    <div class="mt-2">
                        <span class="text-sm text-stone-900">Permasalahan</span>
                        <textarea class="flex w-[450px] text-sm font-semibold text-stone-900 border rounded-lg p-1 outline-none"rows="3"
                            readonly>{{ $complaint_response->problem }}</textarea>
                    </div>
                </div>
                <div class="border rounded-lg p-2 bg-stone-200 w-full">
                    <div class="mt-2">
                        <span class="text-sm text-stone-900">Penyelesaian</span>
                        <textarea class="flex w-[450px] text-sm font-semibold text-stone-900 border rounded-lg p-1 outline-none"rows="5"
                            readonly>{{ $complaint_response->problem_solving }}</textarea>
                    </div>
                </div>
            </div>
            <!-- View Create end -->
