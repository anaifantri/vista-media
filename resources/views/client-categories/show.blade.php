...
@section('container')
    <?php
    $createdDate = strtotime($client_category->created_at);
    $updatedDate = strtotime($client_category->updated_at);
        ...
        <div class="p-4 w-[350px] h-[500px] border rounded-lg">
            ...
            <div>
                ...
                <div class="flex justify-center mt-2">
                    <a href="/marketing/client-categories" class="flex items-center justify-center btn-primary mx-1">
                    ...
                    <a href="/marketing/client-categories/{{ $client_category->id }}/edit"
                    ...
                    <form action="/marketing/client-categories/{{ $client_category->id }}" method="post" class="d-inline">
                        @method('delete')
                        @csrf
                        @if ($client_category->clients()->exists())
                            <button class="flex items-center justify-center btn-danger mx-1"
                                onclick="return confirm('Berelasi dengan {{ count($client_category->clients) }} data pada tabel data klien, apakah anda yakin ingin menghapus data katagori klien dengan nama {{ $client_category->name }} sekaligus menghapus data-data yang berelasi?')">
                        ...
                        @else
                            <button class="flex items-center justify-center btn-danger mx-1"
                                onclick="return confirm('Apakah anda yakin ingin menghapus katagori klien dengan nama {{ $client_category->name }} ?')">
                                <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd"
                                    stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
